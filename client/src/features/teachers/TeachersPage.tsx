...
export function TeachersPage() {
  ...
  const canManageBranchTeachers =
    isTeacher(user) && hasPermission(user, Permission.MANAGE_BRANCH_TEACHERS);
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      ...
      if (apiFilters.branches) {
        apiFilters.branch = apiFilters.branches;
        delete apiFilters.branches;
      }
  ...
  function sharesABranch(teacher: Teacher): boolean {
    const teacherBranchIds = teacher.branches
      .map((branch) => refId(branch))
      .filter((id): id is string => !!id);
  ...
  async function handleDeactivate() {
    ...
    try {
      ...
      toast.error(
        getApiErrorMessage(error, "Could not deactivate the teacher"),
      );
    }
  }
  ...
  const columns: DataTableColumn<Teacher>[] = [
    ...
    },
    {
      ...
      header: "Designation",
      filterable: true,
      filterOptions: Object.entries(DESIGNATION_LABELS).map(
    ...
    },
    {
      ...
      render: (teacher) => (
        <div className="flex flex-wrap gap-1">
          {teacher.specializedCourses.length === 0 && (
