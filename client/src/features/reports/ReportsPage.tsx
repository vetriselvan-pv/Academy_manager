...
export function ReportsPage() {
  const { data: enrollments, isLoading: enrollmentsLoading } = useQuery({
    ...
    queryFn: () => enrollmentsApi.list({ status: EnrollmentStatus.ACTIVE }),
  });
  ...
  const totalFeesCollected = allEnrollments.reduce(
    (sum, e) => sum + (e.feePaid || 0),
    0,
  );
  ...
  const activePct = totalEnrollments
    ? (activeEnrollments.length / totalEnrollments) * 100
    : 0;
  const completedPct = totalEnrollments
    ? (completedEnrollments.length / totalEnrollments) * 100
    : 0;
  const cancelledPct = totalEnrollments
    ? (cancelledEnrollments.length / totalEnrollments) * 100
    : 0;
  ...
  return (
    <div>
      <PageHeader
        title="Reports"
        description="A quick snapshot of enrollment and fee activity across your branch(es)."
      />
      <p className="-mt-4 mb-6 text-xs text-slate-400">
        First-version report, composed from enrollment and student data.
        Dedicated reporting endpoints and richer breakdowns are expected to land
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <StatCard
              ...
              value={cancelledEnrollments.length}
              icon={XCircle}
              tone="amber"
            />
            <StatCard
